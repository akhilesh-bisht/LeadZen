...
export const LeadTable: React.FC<LeadTableProps> = ({
  ...
  const getRepColor = (name?: string | null) => {
    ...
    const found = teamMembers.find((m) => m.name.toLowerCase() === name.toLowerCase());
          ...
          <table className="w-full text-left border-collapse text-xs">
            <thead>
              <tr className="bg-slate-950/60 border-b border-slate-800 text-slate-400 uppercase tracking-wider font-semibold text-[10px]">
                ...
                <th className="py-3.5 px-3.5 text-right min-w-[110px]">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-800/80 text-slate-300">
              {leads.map((lead) => {
                const isSelected = selectedIds.includes(String(lead._id));
                const isStatusUpdating = isUpdatingStatusId === String(lead._id);
                const isAssignUpdating = isUpdatingAssignId === String(lead._id);
                ...
                return (
                  ...
                  >
                    ...
                    {/* City & Address */}
                    <td className="py-3.5 px-3">
                      <div className="font-semibold text-slate-200">{lead.city}</div>
                      <div className="text-[11px] text-slate-400 line-clamp-1 max-w-[140px]" title={lead.address || ''}>
                        {lead.address || '-'}
                      </div>
                    </td>
                    ...
                    <td className="py-3.5 px-3">
                      {lead.phone ? (
                        <div className="flex items-center gap-1.5">
                          ...
                          <button
                            onClick={(e) => handleCopy(e, lead.phone!, `phone-${lead._id}`)}
                        ...
                        <span className="text-slate-500 italic text-[11px]">Unlisted</span>
                      )}
                    </td>
                      ...
                      <div className="relative inline-block text-left">
                        <button
                          ...
                          onClick={() => {
                            ...
                            setActiveAssignDropdownId(
                              activeAssignDropdownId === String(lead._id) ? null : String(lead._id)
                            );
                          }}
                          ...
                          {isAssignUpdating ? (
                            ...
                            <div className="flex items-center gap-1.5">
                              <span
                                ...
                                style={{ backgroundColor: getRepColor(lead.assignedTo) }}
                              >
                        ...
                        {activeAssignDropdownId === String(lead._id) && (
                          <div className="origin-top-left absolute left-0 mt-1 w-44 rounded-xl shadow-xl bg-slate-900 border border-slate-800 divide-y divide-slate-800 z-50 p-1">
                            ...
                            <button
                              onClick={() => handleAssignSelect(String(lead._id), '')}
                              className={`w-full text-left px-2.5 py-1.5 text-xs rounded-lg flex items-center justify-between hover:bg-slate-800 transition-colors cursor-pointer ${
                                !lead.assignedTo ? 'font-semibold text-white bg-slate-800/80' : 'text-slate-400'
                              }`}
                            >
                              <span>Unassign (Queue)</span>
                              {!lead.assignedTo && <Check className="w-3.5 h-3.5 text-indigo-400" />}
                            </button>

                            {teamMembers.map((member) => (
                              <button
                                key={member.id}
                                onClick={() => handleAssignSelect(String(lead._id), member.name)}
                                className={`w-full text-left px-2.5 py-1.5 text-xs rounded-lg flex items-center justify-between hover:bg-slate-800 transition-colors cursor-pointer ${
                                  lead.assignedTo?.toLowerCase() === member.name.toLowerCase()
                      ...
                      <div className="relative inline-block text-left">
                        <button
                          ...
                          onClick={() => {
                            ...
                            setActiveStatusDropdownId(
                              activeStatusDropdownId === String(lead._id) ? null : String(lead._id)
                            );
                          }}
                        ...
                        {activeStatusDropdownId === String(lead._id) && (
                          <div className="origin-top-left absolute left-0 mt-1 w-36 rounded-xl shadow-xl bg-slate-900 border border-slate-800 divide-y divide-slate-800 z-50 p-1">
                            {(['new', 'contacted', 'interested', 'not_interested', 'converted'] as LeadStatus[]).map(
                              (st) => (
                                <button
                                  key={st}
                                  onClick={() => handleStatusSelect(String(lead._id), st)}
                                  className={`w-full text-left px-2.5 py-1.5 text-xs rounded-lg flex items-center justify-between hover:bg-slate-800 transition-colors cursor-pointer ${
                                    lead.status === st ? 'font-semibold text-white bg-slate-800/80' : 'text-slate-300'
                                  }`}
                                >
                                  <span>
                                    {st === 'new'
                    ...
                    <td className="py-3.5 px-3">
                      {lead.rating !== null && lead.rating !== undefined ? (
                        <div className="flex items-center gap-1">
                          ...
                          <span className="font-semibold text-slate-200">{lead.rating.toFixed(1)}</span>
                        </div>
                      ) : (
                        <span className="text-slate-500">-</span>
                      )}
          ...
          return (
            <div
              ...
              className={`p-4 rounded-2xl bg-slate-900/90 border transition-all space-y-3 ${
                isSelected ? 'border-indigo-500 bg-indigo-500/10 shadow-sm' : 'border-slate-800/90'
              }`}
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-2 border-t border-slate-800 text-xs">
                {lead.phone ? (
                  ...
                  <div className="p-2 text-slate-500 italic text-[11px]">No phone listed</div>
                )}
